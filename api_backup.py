...
def calculate_elasticity_pricing():
    ...
    try:
        ...
        # Optional: Create or fetch RouteConfig object if other parts of the model need it.
        # For now, the ElasticityBasedPricingModel primarily uses route_id for parameter lookup.
        # If a full RouteConfig object is still passed, the model can use its .id or .route_id
        
        route_config_for_model = None
        if 'origin' in request_data and 'destination' in request_data: # 8个空格
            origin = request_data.get('origin')                         # 12个空格
            destination = request_data.get('destination')                  # 12个空格
            distance = float(request_data.get('distance', 1200))              # 12个空格
            competition_level = request_data.get('competition_level', 'medium') # 12个空格
            popularity = request_data.get('popularity', 'medium')              # 12个空格
            season_factor = float(request_data.get('season_factor', 1.0))          # 12个空格
            flight_type = request_data.get('flight_type', '干线')                  # 12个空格
            route_info_dict = {
            ...
            if final_route_id is not None: # If we have a route_id, include it for consistency
                route_info_dict['id'] = final_route_id 
            
            route_config_for_model = RouteConfig(route_info=route_info_dict) # 12个空格
            # Aircraft association (if provided and needed by RouteConfig)  # 12个空格
            if request_data.get('aircraft_type'):                       # 12个空格
                aircraft_type = request_data.get('aircraft_type')       # 16个空格
                # Simplified aircraft config part, assuming AircraftConfig can be basic for elasticity model
                # or that it's fetched properly if complex details are needed.
                # This part might need to align with how aircraft_configs global is managed.
                default_aircraft_config_details = {'max_payload': 15000, 'max_volume': 90000000} # 16个空格
                found_global_aircraft = aircraft_configs.get(aircraft_type) # 16个空格
                if found_global_aircraft:                               # 16个空格
                    route_config_for_model.set_aircraft(found_global_aircraft) # 20个空格
                else:                                                   # 16个空格
                    aircraft_details_from_req = request_data.get('aircraft_config', default_aircraft_config_details) # 20个空格
                    temp_aircraft = AircraftConfig(aircraft_type, config=aircraft_details_from_req) # 20个空格
                    route_config_for_model.set_aircraft(temp_aircraft) # 20个空格
        elif final_route_id and DB_ROUTES_AVAILABLE:                # 8个空格 (与第一个if对齐)
            # If only route_id is given, but model might need a RouteConfig object
            route_db_data = get_route_by_id_from_db(final_route_id)     # 12个空格
            if route_db_data:                                           # 12个空格
                route_config_for_model = RouteConfig(route_info=route_db_data) # 16个空格
                # Potentially associate aircraft if logic requires
                # This depends on whether ElasticityModel uses RouteConfig for more than just ID

        # Get override parameters from request for the model
        initial_prices_override = request_data.get('initial_prices')
        price_elasticity_override = request_data.get('price_elasticity') # This can be dict or pre-shaped np.array
        base_demands_override = request_data.get('base_demands')
        ...
        num_segments = int(request_data.get('num_segments', 1)) # Elasticity model usually 1 segment

        # Convert price_elasticity_override if it's a dict from JSON to the NumPy array format the model might expect
        # if no direct dict support in model for this specific param for np array self.price_elasticity.
        # The model's _initialize_parameters_with_database_fallback was updated to handle dict for elasticity.
        # So, direct pass-through of the dict should be fine.

        logger.info(f"Instantiating ElasticityBasedPricingModel with route_id: {final_route_id}")
        model = ElasticityBasedPricingModel(
            route_id=final_route_id, # Crucial for DB param loading
            route_config=route_config_for_model, # Pass RouteConfig if available/needed
            booking_period=booking_period,
            initial_prices=initial_prices_override, 
            price_elasticity=price_elasticity_override, # Model now handles dict or np.array
        ...
        if demand_details_list: # demand_details_list contains segment info, typically we only have 1 segment (n=0) for elasticity model on UI
            ...
            for detail in demand_details_list:
                cargo_type_name = detail.get("cargo_type")
...
def calculate_gametheory_pricing():
    ...
    try:
        ...
        p1_nash_prices_all_periods = []
        p2_nash_prices_all_periods = []
        try:
            logger.debug(f"博弈论API: 调用 find_nash_equilibrium (T_periods={actual_T_periods}) K_C1={company1.k_value}, K_C2={company2.k_value}")
            ...
            logger.error(f"博弈论API: 调用 find_nash_equilibrium 时出错: {e}")
            logger.error(traceback.format_exc())
            return jsonify({"error": f"博弈均衡主计算失败: {str(e)}"}), 500
    ...
    try:
        ...
        try:
            ...
            if unit_price_kg is not None:
                ...
                logger.warning("Elasticity model could not determine a price.")
