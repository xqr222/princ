...
class AirCargoCompetitiveModel:
    ...
    def __init__(self, company_id, W=None, route_info=None, route_config=None, 
        ...
        if route_id is not None and GT_DATABASE_ACCESS_AVAILABLE:
            ...
            else: # This else pairs with 'if db_params:'
        ...
        if self.route_config:
            ...
            if self.route_config.aircraft and current_W_value is None: # Only set W if not provided in __init__
                ...
                else: # This else corresponds to the inner if/elif about capacity_details
                    logger.warning(f"Company {self.company_id}: Could not determine max_payload from route_config.aircraft.get_capacity(). Will use default/passed W if not already set.")
            # If self.route_config.aircraft is None or current_W_value was already provided,
            # current_W_value remains as is (either from __init__ or still None).
            # No specific 'else' needed here for the 'if self.route_config.aircraft and current_W_value is None:'

        elif route_info: # This elif corresponds to 'if self.route_config:'
            self.route_info = route_info
            # current_W_value remains as passed or None
        else: # This else corresponds to 'if self.route_config:'
            # no self.route_config and no route_info passed, and not creating a default route_config here anymore
